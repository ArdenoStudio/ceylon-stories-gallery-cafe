...
const HeroSection = React.forwardRef<HTMLDivElement, HeroSectionProps>(
  ({ className, logo, slogan, title, subtitle, hours, callToAction, secondaryCallToAction, backgroundImage, accentImage, establishedYear, contactInfo, ...props }, ref) => {
    ...
    const itemVariants: Variants = {
      hidden: prefersReducedMotion ? { opacity: 0 } : { y: 22, opacity: 0 },
      visible: {
        y: 0,
        opacity: 1,
    ...
    return (
      ...
      >
        ...
        <motion.div
          ...
          initial={prefersReducedMotion ? { opacity: 0.7, scale: 1 } : { opacity: 0.3, scale: 1.12 }}
          animate={prefersReducedMotion ? { opacity: 0.7, scale: 1 } : { opacity: 0.7, scale: 1 }}
          transition={{ duration: prefersReducedMotion ? 0 : 12, ease: 'linear', repeat: 0 }}
        />
        {/* Single combined overlay: radial vignette + vertical darkening */}
        <div
          className="pointer-events-none absolute inset-0 z-[1]"
          style={{
            background: `
              radial-gradient(ellipse at center, transparent 30%, rgba(15,8,5,0.5) 100%),
              linear-gradient(180deg, rgba(15,8,5,0.4) 0%, rgba(15,8,5,0.15) 30%, rgba(15,8,5,0.15) 70%, rgba(15,8,5,0.55) 100%)
            `,
          }}
        />
        ...
        <div className="relative z-10 mx-auto grid min-h-[100svh] w-full max-w-[1720px] grid-cols-1 lg:grid-cols-12">
          <div className="relative flex flex-col px-6 pb-10 pt-28 sm:px-10 lg:col-span-7 lg:px-[clamp(48px,6vw,124px)] lg:pb-14 lg:pt-36">
            ...
            <motion.header className="relative mb-12 flex items-center justify-between gap-6 lg:mb-16" variants={itemVariants}>
              <div className="flex flex-col gap-3">
                {logo ? (
                  ...
                  <span className="font-display text-[20px] tracking-[0.04em] text-cream-page">CEYLON STORIES</span>
                )}
                {slogan ? (
                  <p className="font-editorial text-[10px] uppercase tracking-[0.28em] text-cream-page/55">
                    {slogan}
                  </p>
                ) : null}
              </div>

              <div className="hidden items-center gap-3 rounded-full border border-gold-leaf/35 bg-ink-deep/40 px-5 py-2.5 backdrop-blur-sm md:inline-flex">
                <span className="h-2 w-2 rounded-full bg-forest/80" />
                <span className="font-editorial text-[10px] uppercase tracking-[0.24em] text-cream-page/65">Open Today</span>
              </div>
            </motion.header>

            <motion.div className="relative max-w-[900px]" variants={containerVariants}>
              <motion.h1
                className="font-display text-[clamp(50px,8vw,118px)] font-light leading-[0.88] tracking-[-0.03em] text-cream-page"
              ...
              <motion.p
                className="max-w-[58ch] font-body text-[16px] leading-[1.75] text-cream-page/70 lg:text-[18px]"
                variants={itemVariants}
              >
                {subtitle}
              </motion.p>
              ...
              {hours ? (
                <motion.p
                  className="mt-8 inline-flex items-center gap-3 rounded-full border border-cream-page/15 bg-ink-deep/40 px-5 py-2.5 font-editorial text-[10px] uppercase tracking-[0.24em] text-cream-page/60 backdrop-blur-sm"
                  variants={itemVariants}
                >
              ...
              <motion.div className="mt-10 flex flex-wrap items-center gap-x-8 gap-y-4" variants={itemVariants}>
                ...
                {secondaryCallToAction ? (
                  <a
                    ...
                    className="group inline-flex items-center gap-2 font-editorial text-[10px] uppercase tracking-[0.24em] text-cream-page/70"
                  >
                    <span className="border-b border-cream-page/25 pb-1 transition-colors group-hover:border-gold-leaf">{secondaryCallToAction.text}</span>
            ...
            <motion.footer className="relative mt-auto pt-12 lg:pt-14" variants={itemVariants}>
              <div className="mb-7 h-px w-full bg-gradient-to-r from-cream-page/10 via-cream-page/25 to-transparent" />
              <div className="grid grid-cols-1 gap-4 text-cream-page/58 sm:grid-cols-3">
