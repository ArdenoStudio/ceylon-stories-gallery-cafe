    ...
    <>
      ...
      <section className="relative w-full bg-cream-page py-[clamp(60px,8vh,120px)] px-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex gap-6 md:gap-10 mb-16 border-b border-mahogany/10 pb-0 overflow-x-auto scrollbar-hide">
            {categories.map((cat) => (
              <button
                key={cat}
                onClick={() => setActive(cat)}
                className={`relative font-editorial text-[10px] tracking-[0.2em] uppercase pb-4 transition-colors duration-300 shrink-0 ${
                  active === cat
                    ? 'text-mahogany'
              ...
              >
                ...
                {active === cat && (
                  <motion.div
                    ...
                    className="absolute bottom-0 left-0 right-0 h-[2px] bg-gold-leaf"
          ...
          <AnimatePresence mode="wait">
            {active !== 'All' && (
              <motion.div
                ...
                className={`mb-12 pb-10 border-b ${active === 'Dilmah Tea' ? 'border-[#007C85]/20' : 'border-mahogany/10'}`}
              >
                <h2 className={`font-display font-light text-[clamp(32px,4vw,52px)] leading-[1] tracking-tight mb-3 ${
                  active === 'Dilmah Tea' ? 'text-[#007C85]' : 'text-mahogany'
                }`}>
                  {active === 'Food' && 'The Kitchen'}
                  {active === 'Beverages' && 'The Bar'}
                  {active === 'Dilmah Tea' && 'The Tea Room'}
                  {active === 'Shisha' && 'The Evening Lounge'}
                </h2>
                <p className="font-body text-sm text-mahogany/55 leading-[1.8] max-w-xl">
                  {active === 'Food' && 'Simple, deliberate dishes made from local ingredients. Every plate tells a story about the island.'}
                  {active === 'Beverages' && 'Hand-crafted drinks — from cold brews steeped overnight to single-origin espresso poured with precision.'}
                  {active === 'Dilmah Tea' && 'Served exclusively through our authorised Dilmah partnership. Each tea prepared to exact temperature and steep time.'}
                  {active === 'Shisha' && 'Available after 6:00 PM. Imported blends and our house signature, served in the amber glow of the lounge.'}
                </p>
                {active === 'Dilmah Tea' && (
                  <div className="flex items-center gap-3 mt-5">
                    <img src="/dilmah-logo.png" alt="Dilmah" className="h-6 opacity-50" />
                    <span className="font-editorial text-[8px] tracking-[0.2em] uppercase text-[#007C85]/50">Authorised Partner</span>
                  </div>
                )}
              </motion.div>
            )}
      ...
      {/* Dilmah Partnership Note — Enhanced */}
      <section className="w-full bg-[#007C85]/[0.04] border-t border-[#007C85]/10 py-16 px-6">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-start gap-8 md:gap-16">
          <div className="shrink-0">
            <img src="/dilmah-logo.png" alt="Dilmah" className="h-10 opacity-60 mb-3" />
            <p className="font-editorial text-[9px] tracking-[0.25em] uppercase text-[#007C85]">
              Authorised Dilmah Partner
            </p>
          </div>
          <div className="flex flex-col gap-4">
            <p className="font-body text-sm text-mahogany/60 leading-relaxed max-w-xl">
            ...
            <div className="flex flex-wrap gap-6 font-editorial text-[8px] tracking-[0.18em] uppercase text-mahogany/35">
              <span>Silver Tips · 75°C · 3 min</span>
              <span>Single Estate · 95°C · 4 min</span>
              <span>Green · 70°C · 2.5 min</span>
              <span>Gallery Chai · 100°C · 5 min</span>
            </div>
          </div>
        </div>
      </section>
