...
const ListIcon = () => (
  <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M9 6H21M9 12H21M9 18H17" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M4.35355 6.35355C4.25979 6.44732 4.13261 6.5 4 6.5C3.86739 6.5 3.74021 6.44732 3.64645 6.35355C3.55268 6.25979 3.5 6.13261 3.5 6C3.5 5.86739 3.55268 5.74021 3.64645 5.64645C3.74021 5.55268 3.86739 5.5 4 5.5C4.13261 5.5 4.25979 5.55268 4.35355 5.64645C4.44732 5.74021 4.5 5.86739 4.5 6C4.5 6.13261 4.44732 6.25979 4.35355 6.35355Z" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M4.35355 12.3536C4.25979 12.4473 4.13261 12.5 4 12.5C3.86739 12.5 3.74021 12.4473 3.64645 12.3536C3.55268 12.2598 3.5 12.1326 3.5 12C3.5 11.8674 3.55268 11.7402 3.64645 11.6464C3.74021 11.5527 3.86739 11.5 4 11.5C4.13261 11.5 4.25979 11.5527 4.35355 11.6464C4.44732 11.7402 4.5 11.8674 4.5 12C4.5 12.1326 4.44732 12.2598 4.35355 12.3536Z" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M4.35355 18.3536C4.25979 18.4473 4.13261 18.5 4 18.5C3.86739 18.5 3.74021 18.4473 3.64645 18.3536C3.55268 18.2598 3.5 18.1326 3.5 18C3.5 17.8674 3.55268 17.7402 3.64645 17.6464C3.74021 17.5527 3.86739 17.5 4 17.5C4.13261 17.5 4.25979 17.5527 4.35355 17.6464C4.44732 17.7402 4.5 17.8674 4.5 18C4.5 18.1326 4.44732 18.2598 4.35355 18.3536Z" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
);

const GridIcon = () => (
  <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M8.6252 13.5762H6.3752C5.38108 13.5762 4.5752 14.3821 4.5752 15.3762V17.6262C4.5752 18.6203 5.38108 19.4262 6.3752 19.4262H8.6252C9.61931 19.4262 10.4252 18.6203 10.4252 17.6262V15.3762C10.4252 14.3821 9.61931 13.5762 8.6252 13.5762Z" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M17.6252 13.5762H15.3752C14.3811 13.5762 13.5752 14.3821 13.5752 15.3762V17.6262C13.5752 18.6203 14.3811 19.4262 15.3752 19.4262H17.6252C18.6193 19.4262 19.4252 18.6203 19.4252 17.6262V15.3762C19.4252 14.3821 18.6193 13.5762 17.6252 13.5762Z" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M8.6252 4.57617H6.3752C5.38108 4.57617 4.5752 5.38206 4.5752 6.37617V8.62617C4.5752 9.62028 5.38108 10.4262 6.3752 10.4262H8.6252C9.61931 10.4262 10.4252 9.62028 10.4252 8.62617V6.37617C10.4252 5.38206 9.61931 4.57617 8.6252 4.57617Z" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M17.6252 4.57617H15.3752C14.3811 4.57617 13.5752 5.38206 13.5752 6.37617V8.62617C13.5752 9.62028 14.3811 10.4262 15.3752 10.4262H17.6252C18.6193 10.4262 19.4252 9.62028 19.4252 8.62617V6.37617C19.4252 5.38206 18.6193 4.57617 17.6252 4.57617Z" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
);

const buttons = [
  { id: 'list' as const, icon: <ListIcon /> },
  { id: 'grid' as const, icon: <GridIcon /> },
];
...
export function InteractiveNav({ view, onViewChange }: Props) {
  ...
  const [buttonStates, setButtonStates] = useState(
    buttons.map(() => ({ hover: false, press: false }))
  );

  useEffect(() => {
    const nav = navRef.current;
    const light = lightRef.current;
    const btns = buttonRefs.current.filter(Boolean) as HTMLDivElement[];
    const allButtonLights = buttonLightsRefs.current.filter(Boolean) as HTMLDivElement[];

    if (!nav || !light || btns.length === 0) return;
    ...
    const calculateIntensity = (event: MouseEvent, innerRadius: number, outerRadius: number) => {
      ...
      if (distance <= innerRadius) return 0;
    ...
    const handleMouseMove = (event: MouseEvent) => {
      ...
      allButtonLights.forEach(el => { el.style.opacity = opacity.toString(); });

      btns.forEach(item => {
        const angle = calculateAngle(item, x, y);
        const scaleY = 10 - easeOutQuint(calculateIntensity(event, 0, lightRadius * 1.4)) * 10;
        const bg = item.querySelector('.lnt-button-bg') as HTMLImageElement;
        if (bg) bg.style.transform = `rotateZ(${angle}deg) scaleY(${scaleY})`;
      });
    };
  ...
  const updateButtonState = (index: number, state: Partial<{ hover: boolean; press: boolean }>) => {
    setButtonStates(current => current.map((s, i) => (i === index ? { ...s, ...state } : s)));
  };

  return (
    <div className="lnt-root">
      <div className="lnt-nav" ref={navRef}>
        {buttons.map((button, index) => {
          const state = buttonStates[index];
          const isActive = view === button.id;
          const cls = [
            'lnt-button',
            state.hover ? 'hover' : '',
            state.press ? 'press' : '',
