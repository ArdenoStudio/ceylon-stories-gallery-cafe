      ...
      <section className="relative w-full bg-cream-page py-[clamp(80px,12vh,160px)] px-6 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto mt-20">
          <motion.div
            ...
            transition={{ duration: 1, ease: [0.16, 1, 0.3, 1] }}
          >
            <p className="font-editorial text-[10px] tracking-[0.25em] uppercase text-center mb-12" style={{ color: TEAL }}>
              A Timeline of Purpose
            </p>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-0 relative">
              {/* Connecting line */}
              <div className="hidden md:block absolute top-[22px] left-[12.5%] right-[12.5%] h-[1px]" style={{ backgroundColor: `${TEAL}25` }} />
              
              {[
                { year: '1988', label: 'Dilmah Founded', desc: 'Merrill J. Fernando packs the first tea under the Dilmah name — breaking 150 years of colonial blending.' },
                { year: '2000', label: 'MJF Foundation', desc: 'The charitable foundation is established, funding schools, hospitals, and livelihoods across tea-growing communities.' },
                { year: '2024', label: 'Partnership Formed', desc: 'Ceylon Stories enters its authorised partnership with Dilmah, becoming one of few independent cafés to serve the reserve collection.' },
                { year: '2025', label: 'Ceylon Stories Opens', desc: 'The gallery café opens on Marine Drive, with the Dilmah tea experience as a cornerstone of its identity.' },
              ].map((milestone, idx) => (
                <motion.div
                  key={milestone.year}
                ...
                >
                  <div className="w-3 h-3 rounded-full mb-4 border-2 bg-cream-page relative z-10" style={{ borderColor: TEAL }} />
                  <p className="font-display text-2xl mb-2" style={{ color: TEAL }}>{milestone.year}</p>
                  <p className="font-editorial text-[8px] tracking-[0.2em] uppercase text-mahogany/70 mb-2">{milestone.label}</p>
                  <p className="font-body text-xs text-mahogany/50 leading-[1.7] max-w-[22ch]">{milestone.desc}</p>
                </motion.div>
              ))}
            </div>
          </motion.div>
